...
func init() {
	config.Add("log", func() map[string]interface{} {
		return map[string]interface{}{
			...
			// "error -- 记录错误信息. Panic 或者 Error. 如数据库连接错误,HTTP 端口 被占用等,一般生成环境使用的等级
			...
			/*--------------滚动日志配置----------------*/
			// 日志文件路径
			"filename": config.Env("LOG_NAME", "storage/logs/log.log"),
			// 每个日志文件保存的最大尺寸 单位: M
			"max_size": config.Env("LOG_MAX_SIZE", 64),
			// 最多保存日志文件数, 0为不限, MaxAge 到了还是会删
			"max_backup": config.Env("LOG_MAX_BACKUP", 5),
			// 最多保存多少天, 7 表示一周前的日志会被删除, 0表示不删
