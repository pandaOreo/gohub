...
type UserUpdatePhoneRequest struct {
	...
	VerifyCode string `valid:"verify_code" json:"verify,omitempty"`
}
...
type UserUpdateAvatarRequest struct {
	Avatar *multipart.FileHeader `valid:"avatar" json:"avatar,omitempty"`
}
...
func UserUpdateAvatar(data interface{}, c *gin.Context) map[string][]string {
	rules := govalidator.MapData{
		...
		"file:avatar": []string{
			"ext:png,jpg,jpeg", "size:20971520", "required",
		},
	}
	messages := govalidator.MapData{
		"file:avatar": []string{
			"ext:ext头像只能上传png,jpg,jpeg 任意一直的图片",
