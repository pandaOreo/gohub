...
func InitLogger(filename string, maxSize, maxBackup, maxAge int, compress bool, logType string, level string) {
	...
	// 设置日志等级, 具体请见 config/log.go
	logLevel := new(zapcore.Level)
	if err := logLevel.UnmarshalText([]byte(level)); err != nil {
		fmt.Println("日志初始化错误,日志爱级别设置有误.请修改 config/log.go 文件中的log.level配置项")
	}
...
func getEncoder() zapcore.Encoder {
	...
	encoderConfig := zapcore.EncoderConfig{
		...
		EncodeCaller:   zapcore.ShortCallerEncoder,     // Caller 短格式, types/converter.go:17, 长格式为绝对路径
	}
...
// LogIf 当 err != nil 时 记录 error 等级的日志
