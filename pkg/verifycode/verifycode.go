...
import (
	...
	"github.com/ZimoBoy/gohub/pkg/mail"
	"github.com/ZimoBoy/gohub/pkg/redis"
	"github.com/ZimoBoy/gohub/pkg/sms"
	"strings"
	"sync"
)
...
var interfaceVerifyCode *VerifyCode

// NewVerifyCode 单例模式获取
func NewVerifyCode() *VerifyCode {
	once.Do(func() {
		interfaceVerifyCode = &VerifyCode{
			Store: &RedisStore{
...
func (vc *VerifyCode) SendSMS(phone string) bool {
	...
	// 方便本地和 API 自动测试
	if !app.IsProduction() && strings.HasPrefix(phone, config.GetString("verifycode.debug_expire_time")) {
		return true
	}
...
func (vc *VerifyCode) generateVerifyCode(key string) string {
	...
	// 将厌女症吗及 KEY (邮箱或手机号) 存放到 Redis 中并设置过期时间
